...
How these \code{id}s are created can be seen the function \code{create\_category\_id()} in \code{columnflow/config\_util.py}.
...
\begin{exercise}{Defining Categories}[h4l/categorization/solution.py]
	Write three new \CCSPStlye{Categorizer} functions in \code{h4l/categorization/default.py} to select, respectively, events with \textit{at least} four electrons, four muons and, lastly, two electrons and two muons.
...
The leaf categories will be created by picking a single category from \textit{each} variable group. Thus, in this example, \code{cat\_incl} and \code{cat\_2j} would not be combined into a leaf category, since they belong to the same variable group, \code{'kinematics'}.
...
In this example, the \code{'channel'} group is incomplete, as events with other leptons in the final state, or a different number of electron other than two, are not represented in the group.
...
A schematic view of the category combinatorics logic is shown in Fig.~\ref{fig:categories}.
\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{figures/categories}
    \caption{Schematic view of the category combinatorics.}
